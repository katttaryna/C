...
int main(){
    ...
    signal(SIGUSR1, SIG_IGN);// игнорирование сигнала (sa_handler задает тип действий процесса)
    ...
    while(1){
        ...
        switch(parametr){
            ...
            case '-':
                ...
                kill(child_pids[number_of_child_pids-1], SIGTERM); //signal terminate
                number_of_child_pids--;
                child_pids = realloc(child_pids, number_of_child_pids * sizeof(pid_t));
            ...
            case 's':
                ...
                if(number_of_child != '\n') kill(child_pids[(number_of_child-'0')], SIGUSR1); //запр вывод инф
            ...
            case 'g':
                ...
                if(number_of_child != '\n'){
                    ...
                    kill(child_pids[(number_of_child-'0')], SIGUSR2); // вывод инфо
                } 
            ...
            case 'p':
                ...
                for(int i = 0; i < number_of_child_pids; i++) kill(child_pids[i], SIGUSR1); // запр вывод конкретного
                alarm(5);
                break;
            case 'q':
                printf("killing all children\n");
