...
int main()
{
    ...
    signal(SIGUSR1, ban); // запрещает вывод информации
    signal(SIGUSR2, disban); //вывод информации
    signal(SIGINT, immediate_stat); //при прерывании ctrl c ядро передает сигнал всем процессам
    while (1)
    {
        signal_from_alarm = 0;
